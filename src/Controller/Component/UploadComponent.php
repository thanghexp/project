...
class UploadComponent extends Component {
	...
	function uploadSoundFile($file, $destination, $name = null, $rules = null, $allowed = null)
	{
		$this->result = false;
		$this->error = false;
		...
		if (!is_null($allowed)) { $this->_allowed = $allowed; } else { $this->_allowed = array('mp3', 'wmp', 'wav'); }
	...
	/**
	 * Upload resume file of Jobseeker
	 *
	 * @param file upload $file
	 * @param destination folder $destination
	 * @param destiantion filename $name
	 * @param  $rules
	 * @param extension file: $allowed 
	 */
	
	function uploadFile($file, $destination, $name = null, $rules = null, $allowed = null)
	{
		$this->result = false;
		$this->error = false;
		...
		if (!is_null($allowed)) { $this->_allowed = $allowed; } else { 
			$this->_allowed = array('pdf'); 
		}
	...
	function image ($file, $type, $size, $output = NULL, $quality = NULL) {
		...
		$size = $uploadSize;
		if($maxH) $maxH = $uploadHeight;
		if($maxW) $maxW = $uploadWidth;
		// -- check sizes
		if (isset($maxScale)) {
			if (!$maxScale) {
		...
		switch ($type) {
			...
			case 'resizemin':
				...
				//end new code
				{
					...
					elseif (($ratioY * $uploadWidth) > $maxW) {
						$newX = $maxW;
						$newY = ceil($ratioX * $uploadHeight);
					} else {
						$newX = ceil($ratioY * $uploadWidth);		
	...
	function resize($width, $height, $resizeLocation, $originalLocation, $scale = 0, $type = 'resize') {
		...
		if ($type == 'resizemin') {
			...
			if (($originalWidth == $width) && ($originalHeight == $height)) {
				$newX = $originalWidth;
				$newY = $originalHeight;
			} else if (($ratioX * $originalHeight) > $height) {
				$newX = $width;
